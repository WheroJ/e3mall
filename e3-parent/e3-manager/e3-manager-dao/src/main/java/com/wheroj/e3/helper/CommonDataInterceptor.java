...
public class CommonDataInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		//获取配置参数
		String createDateColumn, updateDateColumn,versionColumn,
		       createByColumn, updateByColumn, createByField, updateByField;
		if(null == props || props.isEmpty()) {
			return invocation.proceed();
		} 
		createDateColumn = props.getProperty("createDateColumn", "created");
		updateDateColumn = props.getProperty("updateDateColumn", "updated");
		...
		if(sqlCmdType == SqlCommandType.UPDATE && updateDateColumn.length() > 0){
			newSql = addUpdateData(parameterObject, originalSql, updateDateColumn, updateByField, updateByColumn);
		}else if(sqlCmdType == SqlCommandType.INSERT && createDateColumn.length() > 0){
			newSql = addInsertData(parameterObject, originalSql, createDateColumn, versionColumn, createByField, createByColumn, updateDateColumn);
		}
		//修改原始sql
		if(newSql.length() > 0){
	...
	private String addInsertData(Object parameterObject, String originalSql, 
			String createDateColumn, String versionColumn, 
		...
		String sql = originalSql;
		       ...
		       sql = addInsertData(parameterObject, sql, updateDateColumn, null,            new TimestampValue(TIMESTAMP_FORMAT.format(new Date())));
//		       sql = addInsertData(parameterObject, sql, versionColumn,    null,            null);//new LongValue(1L)
//		       sql = addInsertData(parameterObject, sql, createByColumn,   createByField,   null);
	...
	private String addUpdateData(Object parameterObject, 
			String originalSql, String updateDateColumn,
